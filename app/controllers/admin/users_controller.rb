class Admin::UsersController < AdminController
  ...
  def show
    ...
    @user_stats = {
      ...
      last_login: @user.current_sign_in_at || @user.created_at
    }
  end
  ...
  def create
    ...
    respond_to do |format|
      if @user.save
        ...
        format.html { redirect_to admin_user_path(@user), notice: t('users.created') }
  ...
  def update
    ...
    respond_to do |format|
      if @user.update(user_params.except(:password, :password_confirmation).merge(password_params))
        format.html { redirect_to admin_user_path(@user), notice: t('users.updated') }
  ...
  def destroy
    ...
    @user.destroy
    redirect_to admin_users_path, notice: t('users.deleted')
  end

  def activate
    @user.update!(active: true)
    redirect_to admin_user_path(@user), notice: t('users.activated')
  end

  def deactivate
    if @user.admin? && User.admins.active.count <= 1
      redirect_to admin_user_path(@user), alert: 'No se puede desactivar el último administrador activo.'
      return
    end

    @user.update!(active: false)
    redirect_to admin_user_path(@user), notice: t('users.deactivated')
  end
